...
from src.lfgp_similar_hqlq import LFGP
...
tmp_mean = []
l=[]
U_record = np.zeros((n_task, 10))
V_record = np.zeros((n_worker, 5, 10))
...
for s in range(iters):
    ...
    model = LFGP(lf_dim=5, n_worker_group=5, lambda1 = 1, lambda2_0 = 1, lambda2_1 = 2)
    peer_model = peerA(rating, 5,n_worker)
    ...
    for i in range(len(rating[:, 0])):    
     ...
     # for the purpose of reproducibility, fix the seed
    oracle = np.array([[0,1],
                       [3,4]])
    A, B, U, V = model._mc_fit(rating, key = label, epsilon=1e-5, maxiter=maxiter, verbose=1)
    #U_record[:, s] = U
    #V_record[:, :, s] = V
    ...
    U_mv_by_group = model._mc_infer(rating)
    U_oracle_worker = model.oracel_worker_group(rating,worker_label,label)
    #U_honest_infer,_,_,_ = model._mc_honest_infer(rating, label)
    #group_top2, group_top2_label, proportions_group = model._mc_infer_top2(rating, label)
    #task_top2, task_top2_label, proportions_task = model._mc_infer_by_task_top2(rating, label)

    tmp[0,s] = np.mean(U_new == label)
    tmp[1,s] = np.mean(U_mv_by_task == label)
    tmp[2,s] = np.mean(U_mv_by_group == label)
    tmp[3,s] = np.mean(U_oracle_worker == label)
    #tmp[3,s] = task_top2
    #tmp[4,s] = group_top2
    #diff_prop = model.proportions(U_mv_by_task, label, worker_label)
    #tmp.append(np.mean(U == label)))
    #model.diagnosis(worker_label, rating)
    #model.distance_graph(centers, worker_label)
    
    MV = peer_model._MV()
    GLAD_hard, GLAD_prob = peer_model._GLAD()
    DS = model._init_task_member_ds(rating)
    
    tmp[4,s] = np.mean(MV == label)
    tmp[5,s] = np.mean(GLAD_hard == label)
    tmp[6,s] = np.mean(DS[:,1] == label)
