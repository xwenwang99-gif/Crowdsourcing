...
for seed in range(1):
    ...
    for s in range(1):        
        ...
        oracle = np.array([[0,1],
                           [3,4]])
        A, B, U, V, centers = model._mc_fit(rating, key = label, epsilon=1e-5, maxiter=50, verbose=1, threshold = 0.5)
        U_record[:, s] = U
        V_record[:, :, s] = V
        ...
        tmp_grouping.append(np.mean(U_new == label))
        tmp_mv_by_task.append(np.mean(U_mv_by_task == label))
        tmp_mv_by_group.append(np.mean(U_mv_by_group == label))
        ...
        tmp.append(np.mean(U_mv_by_task == label))
        tmp.append(np.mean(U_mv_by_group == label))
        tmp.append(task_top2)
        tmp.append(group_top2)
        #tmp.append(np.mean(U == label)))
