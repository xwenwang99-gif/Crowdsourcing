...
for i in range(3):
    ...
    model = LFGP(lf_dim=5, n_worker_group=7, lambda1 = 1, lambda2_0 = 1, lambda2_1 = 2)
    ...
    pred_group = U.astype(int)        # shape (n_task,)
    ...
    for g in range(n_task_groups):
        ...
        plt.hist(abs_vals, bins=40, density=True, alpha=0.8, edgecolor="black")
        plt.axvline(np.partition(abs_vals, -m)[-m], color="red", linestyle="--",
                    label=f"Top-{m} cutoff")
        plt.title(f"Task Group {g}: |Leading Eigenvector| Distribution")
        plt.xlabel("|v_i|")
        plt.ylabel("Density")
        plt.legend()
        plt.tight_layout()
    ...
    '''
        # --- optional: visualize agreement for this predicted group ---
        plt.figure(figsize=(6, 5))
        sns.heatmap(agreement_g, vmin=0, vmax=1, cmap="viridis")
        plt.title(f"Agreement (Pred group {g})")
        plt.tight_layout()
        plt.show()
    
        # reordered heatmap by eigenvector ordering
        order = np.argsort(leading)[::-1]
        plt.figure(figsize=(6, 5))
        sns.heatmap(agreement_g[np.ix_(order, order)], vmin=0, vmax=1, cmap="viridis")
        plt.title(f"Agreement reordered (Pred group {g})")
        plt.tight_layout()
        plt.show()
    '''
    
    #########################################
    # 2. Task labeling accuracy
    #########################################
    ...
    task_accuracy.append(np.mean(task_label_pred == label))
