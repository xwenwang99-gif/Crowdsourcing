# -*- coding: utf-8 -*-
"""
Created on Tue Sep  3 13:31:38 2024

@author: wangl
"""

from src.lfgp_worker_oracle import LFGP
import numpy as np
import warnings
...
for seed in range(1):
    ...
    tmp = []
    l=[]
    assignment_write = np.zeros((5,2,10))
    for s in range(3):
        np.random.seed(s) # for the purpose of reproducibility, fix the seed
        model._mc_fit(rating, worker_key = worker_label, key = label, epsilon=1e-4, maxiter=50, verbose=0)
        
        pred_label, task_group_label, candidate, assignment = model._mc_infer(rating, label)
        tmp.append(np.mean(pred_label[:, 1] == label))
        tmp.append(np.mean(task_group_label == label))
        
        assignment_write[:,:,s] = assignment
    acc[seed] = np.max(tmp)
