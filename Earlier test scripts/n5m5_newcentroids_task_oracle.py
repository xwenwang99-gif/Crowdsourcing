...
from src.lfgp_task_oracle import LFGP
import numpy as np
import warnings
...
for seed in range(1):
    ...
    tmp = []
    l=[]
    assignment_write = np.zeros((5,2,3))
    for s in range(3):
        ...
        pred_label, worker_pred_label, candidate, assignment = model._mc_infer(rating, worker_label)
        tmp.append(np.mean(pred_label[:, 1] == label))
        tmp.append(np.mean(worker_pred_label == worker_label))
