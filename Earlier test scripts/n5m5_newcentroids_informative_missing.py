...
for seed in range(1):
    ...
    for i in range(n_task):
        task_group = i//(n_task//5)
        sub_worker = []
        for j in range(5):
            if(high_quality[task_group, j]==1):
                sub_worker.extend(np.sort(np.random.choice(range(j*(n_worker//5),(j+1)*(n_worker//5)), size=int((n_worker//5)*high_prob), replace=False)))
            else:
                sub_worker.extend(np.sort(np.random.choice(range(j*(n_worker//5),(j+1)*(n_worker//5)), size=int((n_worker//5)*low_prob), replace=False)))

        tmp = np.zeros((len(sub_worker), 3))
        tmp[:, 0] = i
        tmp[:, 1] = sub_worker
