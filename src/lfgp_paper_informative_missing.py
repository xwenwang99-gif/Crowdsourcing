...
class LFGP():
    ...
    def _mc_infer(self, data, key):
        ...
        assignment = np.zeros((self.n_task_group, self.n_worker_group, len(gA)))
        
        candidate = np.zeros((self.n_task_group, self.n_worker_group, len(gA)))
        
        label_assignment = np.zeros((self.n_task_group))
        ...
        for i in gA:
            ...
            flat_indices = np.argsort(candidate_temp.ravel())[-3:]  # Top 3 flattened indices
            top_3_indices = np.unravel_index(flat_indices, candidate_temp.shape)  # Convert to 2D indices
            top_3_values = candidate_temp[top_3_indices]  # Get the top 3 values
            
            
            top_3_percentages = top_3_values / np.sum(top_3_values)  # Percentage for top 3 values
            
            # Assign labels for the top 3 values
            for rank, (task_group, worker_group) in enumerate(zip(top_3_indices[0], top_3_indices[1])):
                assignment[task_group, worker_group, i] = top_3_percentages[rank] 
                '''
            assignment_tmp = np.sum(assignment[:, :, i], axis = 1)
            x = np.argmax(assignment_tmp)
            label[self.U == x, 1] = i
            label_assignment[x] = i
            '''
        assignment_tmp = np.sum(assignment, axis = 1)
        x = np.argmax(assignment, axis = 0)
        
        return candidate, assignment, label, assignment_tmp
