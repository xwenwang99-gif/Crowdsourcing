...
class LFGP():
    ...
    def _init_mc_params(self, data, worker_key, scheme):
        ...
        if scheme == "mv":
            ...
            worker_member = np.zeros((self.n_worker, 2))
            worker_member[:, 0] = np.unique(data[:, 1])
            worker_member[:, 1] = worker_key
    ...
    def _mc_fit(self, data, worker_key, key, scheme="ds", maxiter=50, epsilon=1e-5, verbose=0):
        ...
        self._init_mc_params(data, worker_key, scheme=scheme)

        iter = 0
        loss_cur = self._mc_loss_func(data)
        err = 1
        loss = np.zeros(maxiter+1)
        loss[0] = loss_cur
        ...
        while err > epsilon:
            ...
            for i in range(self.n_record):
                ...
                loss_tmp += - np.log(prob[int(data[i, 2])])
            if verbose > 0:
                print("Iter: {0}, loss: {1}".format(iter, loss_cur))
    ...
    def _mc_infer(self, data, key):
            ...
            for i in gA:
                ...
                label[self.U == x[0,0]] = i
                

            group_infer = self.label_swap(self.U, key)
    
            return label, group_infer, candidate, assignment
