# -*- coding: utf-8 -*-
"""
Created on Thu Sep 12 22:10:20 2024
...
class LFGP():
    ...
    def _init_mc_params(self, data, key, scheme):
        ...
        if scheme == "mv":
            ...
            task_member = np.zeros((self.n_task, 2))
            task_member [:, 0] = np.unique(data[:, 0])
            task_member [:, 1] = key
    ...
    def _mc_fit(self, data, key, scheme="ds", maxiter=50, epsilon=1e-5, verbose=0):
        ...
        self._init_mc_params(data, key, scheme=scheme)
    ...
    def _mc_infer(self, data, worker_key):
            gA, alpha = npi.group_by(self.U).mean(self.A, axis=0)
            gB, beta = npi.group_by(self.V).mean(self.B, axis=0)
            ...
            group_infer = self.label_swap(self.V, worker_key)
