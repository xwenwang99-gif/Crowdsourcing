...
'''
    ...
    def jaccard_similarity(self, vec1, vec2):
        """
        Compute Jaccard similarity for two binary 0/1 vectors of equal length.
        """
    ...
    def _mc_fit(self, data, key, scheme="ds",maxiter=50, epsilon=1e-5, verbose=0, threshold = 0.6):
        ...
        for iter_count in range(maxiter):
            ...
            for w in range(self.n_worker):
                for group in range(self.n_task_group):
                    ...
                    obs_idx = np.where((data[:, 1] == worker_ids[w]) & (U_prev[data[:, 0].astype(int)] == group))[0]
                    ...
                    if iter_count==0:
                        centroid = np.mean(self.B[w, group, :], axis=0)
                    else:
            ...
            similarity_matrix = np.zeros((self.n_task_group, self.n_task_group))

            for t1 in range(self.n_task_group):
                for t2 in np.arange(t1+1, self.n_task_group):
                    #similarity = self.jaccard_similarity(self.V[:, t1], self.V[:, t2])
                    similarity = abs(np.mean(V_cur[:, t1] == V_cur[:, t2]))
                    similarity_matrix[t1, t2] = similarity
            
            row_idx, col_idx = np.where(similarity_matrix > threshold)
            exceed_indices = np.column_stack((row_idx, col_idx))
            print(exceed_indices)
            
            for t in range(exceed_indices.shape[0]):
                B_slice_0 = self.B[:, exceed_indices[t, 0], :]
                B_slice_1 = self.B[:, exceed_indices[t, 1], :]
                B_slice = np.hstack((B_slice_0, B_slice_1))
                V_cur[:, exceed_indices[t, 0]], cluster_temp = self.new_kmeans(B_slice, n_clusters=2)
                clusters[:, :, exceed_indices[t, 0]] = cluster_temp[:, :self.lf_dim]
                clusters[:, :, exceed_indices[t, 1]] = cluster_temp[:, self.lf_dim:]
                V_cur[:, exceed_indices[t, 1]] = V_cur[:, exceed_indices[t, 0]]
                #V_cur[:, exceed_indices[t, 0]] = self.label_swap(V_cur[:, exceed_indices[t, 0]], V_prev[:, exceed_indices[t, 0]])
                #V_cur[:, exceed_indices[t, 1]] = self.label_swap(V_cur[:, exceed_indices[t, 1]], V_prev[:, exceed_indices[t, 1]])
    ...
    def oracel_worker_group(self, data, oracel_worker,key):
        task_label = np.zeros(self.n_task)
        for t in range(self.n_task):
            hq_worker = oracel_worker[:,self.U[t]]
            hq_worker = np.where(hq_worker==1)[0]
            data_t = data[np.isin(data[:, 1], hq_worker) & (data[:, 0] == t)]
            task_label[t] = statistics.mode(data_t[:,2])
        return task_label
