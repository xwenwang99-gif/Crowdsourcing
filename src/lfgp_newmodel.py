...
class LFGP():
    ...
    def new_kmeans(self, X, n_clusters):
        max_iter=300
        tol=1e-4
        ...
        cluster_centers[1:] = X[np.random.choice(n_samples, n_clusters - 1, replace=False)]
        ...
        for _ in range(max_iter):
            ...
            # Update cluster centers except for the fixed one at 0
    ...
    def _mc_fit(self, data, key, scheme="ds", maxiter=50, epsilon=1e-5, verbose=0):
        ...
        while err > epsilon:
            ...
            for id, _ in enumerate(task):
                ...
                centers, task_worker_groups[id] = self.new_kmeans(obsB, n_clusters=2)
    ...
    def _mc_infer(self, data, worker_label):
            ...
            worker_group = self.label_swap(self.V, worker_label)
            label[:, 1] = self.U
    
            return label, worker_group, self.V
